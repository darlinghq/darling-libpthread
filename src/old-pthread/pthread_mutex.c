...
#ifndef DARLING_BUILDING_OLD_PTHREAD
// XXX remove
void
cthread_yield(void) 
{
...
_pthread_mutex_corruption_abort(_pthread_mutex *mutex)
{
	...
	PTHREAD_CLIENT_CRASH(0, "pthread_mutex corruption: mutex owner changed in the middle of lock/unlock");
#else
...
int _pthread_mutex_droplock(_pthread_mutex *mutex, uint32_t *flagsp, uint32_t **pmtxp, uint32_t *mgenp, uint32_t *ugenp) {
	return _pthread_mutex_unlock_updatebits(mutex, flagsp, pmtxp, mgenp, ugenp);
};
